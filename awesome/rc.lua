...
local function client_menu_fn(cli)
	...
	return function (cli)
		...
		if state then
			floating_item[1] = "unset floating"
			floating_item[3] = beautiful.layout_max
		else
			floating_item[1] = "set floating"
			floating_item[3] = beautiful.layout_floating
		end
		table.insert(terms, floating_item)
		table.insert(terms, {"move to", move_to_tags_menu})
		table.insert(terms, {
							 "close",
							 function ()
								 cli:kill()
...
local function create_close_widget()
	local ret = imagebox()
	img = beautiful["titlebar_close_button_focus"]
	ret:set_image(img)
...
for i = 1, 9 do
	table.insert(move_to_tags_menu, {tostring(i),
												function (cur_client)
...
awful.rules.rules = {
	...
	{ rule = { },
	  properties = { border_width = 5,--beautiful.border_width,
...
client.connect_signal("request::titlebars", function(c)
								 ...
								 awful.titlebar(c) : setup {
									 ...
									 { -- Middle
										 ...
										 --layout = wibox.layout.flex.forced_width
									 },
									 --[[{ -- Right
